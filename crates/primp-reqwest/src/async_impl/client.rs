...
impl ClientBuilder {
    ...
    pub fn build(self) -> crate::Result<Client> {
        ...
        let resolver = {
            ...
            DynResolver::new(Arc::new(crate::dns::cache::DnsCacheResolver::new(resolver)))
        };
        ...
        #[cfg(feature = "http2")]
        {
            ...
            if let Some(http2_enable_connect_protocol) = config.http2_enable_connect_protocol {
                builder.http2_enable_connect_protocol(http2_enable_connect_protocol);
            }
            ...
            if let Some(data) = config.http2_headers_priority {
                builder.http2_headers_priority(Some(data));
            }
            if let Some(ref order) = config.http2_headers_order {
                builder.http2_headers_order(order.clone());
            }
...
impl Client {
    ...
    pub fn set_proxies(&mut self, proxies: Vec<Proxy>) {
        let proxy_matchers: Vec<ProxyMatcher> = proxies
            .into_iter()
            .map(|p| p.into_matcher())
            .collect();
