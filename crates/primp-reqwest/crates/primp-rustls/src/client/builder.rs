...
impl ConfigBuilder<ClientConfig, WantsVersions> {
    ...
    ///
    /// Note: ECH GREASE mode can work with both TLS 1.2 and TLS 1.3 enabled.
    /// The protocol version will be negotiated during the handshake.
    ///
    ...
    pub fn with_ech(
        ...
        // Use the default protocol versions (both TLS 1.2 and TLS 1.3 if enabled)
        let mut res = self.with_safe_default_protocol_versions()?;
