...
impl ClientHelloInput {
    pub(super) fn new(
        ...
        // Use extension order seed from browser emulation if available
...
fn emit_client_hello_for_retry(
    ...
    // Add X25519MLKEM768 for post-quantum key exchange compatibility
    // Only add if not already present and if not using browser emulation
    // (browser emulation already has the correct named groups)
    if !named_groups_vec.contains(&NamedGroup::X25519MLKEM768) && config.browser_emulation.is_none() {
        named_groups_vec.insert(0, NamedGroup::X25519MLKEM768);
    }
    ...
    let mut exts = Box::new(ClientExtensions {
        ...
        signature_schemes: Some(
            config
                ...
                .map(|schemes| schemes.iter().copied().collect())
    ...
    if let Some(be) = config.browser_emulation.as_ref() {
        match be.browser_type {
            BrowserType::Chrome | BrowserType::Edge | BrowserType::Opera => {
                // Note: max_fragment_length removed for Chrome 144 JA4 compatibility
                // exts.max_fragment_length = Some(1); // 2^9 = 512 bytes (value 1)
                // Signed certificate timestamp extension for Chrome fingerprinting
                exts.signed_certificate_timestamp = Some(()); // Empty SCT extension
                // Note: padding removed for Chrome 144 JA4 compatibility (target hash d8a2da3f94cd)
                // exts.padding = Some(0);
                ...
                // ALPS extension (0x44cd) for Chrome fingerprinting
                // Chrome 136+ uses new ALPS codepoint (17613 = 0x44cd) instead of old (17513 = 0x4469)
                // Format: u16 total length (6), u8 protocol length (5), "primp" bytes
                exts.unknown_extensions.push((
                    ExtensionType::Unknown(0x44cd),
                    Payload::new(vec![0x00, 0x06, 0x05, 0x70, 0x72, 0x69, 0x6d, 0x70]),
                ));

                // GREASE extension for Chrome fingerprinting (0xbaba)
                exts.unknown_extensions.push((
                    ExtensionType::Unknown(0xbaba),
                    Payload::empty(),
                ));
            ...
            BrowserType::Safari => {
                ...
                // Check if this is Safari 18.5 (needs 0x0015 extension)
                let is_safari_18_5 = be.version.major == 18 
                    && be.version.minor == 5 
                    && be.version.patch == 0;
                ...
                // Only add status_request_v2 extension (0x0015) for Safari 18.5
                if is_safari_18_5 {
                    exts.contiguous_extensions.push(ExtensionType::Unknown(0x0015));
                    exts.unknown_extensions.push((
                        ExtensionType::Unknown(0x0015),
                        Payload::empty(),
                    ));
                }
    ...
    let mut cipher_suites: Vec<_> = config
        ...
        .map(|suites| {
            let result: Vec<_> = suites.iter().copied().collect();
            result
        })
        .unwrap_or_else(|| {
    ...
    if supported_versions.tls12 && config.browser_emulation.is_none() {
        ...
        cipher_suites.push(CipherSuite::TLS_EMPTY_RENEGOTIATION_INFO_SCSV);
            ...
            if let Some(be) = config.browser_emulation.as_ref() {
                if matches!(be.browser_type, BrowserType::Chrome | BrowserType::Edge | BrowserType::Opera) {
                    // Set contiguous extensions for Chrome
                    // ALL extensions must be in this list to control the complete order
                    // Note: ECPointFormats (0x000b) is included for JA4 fingerprint (16 extensions)
                    debug!("Chrome emulation: Setting final contiguous extensions after all extensions added");
                    exts.contiguous_extensions = vec![
                        ExtensionType::ServerName,              // 0x0000 - SNI
                        ExtensionType::ExtendedMasterSecret,    // 0x0017
                        ExtensionType::SessionTicket,           // 0x0023
                        ExtensionType::SignatureAlgorithms,     // 0x000d
                        ExtensionType::StatusRequest,           // 0x0005
                        ExtensionType::EllipticCurves,          // 0x000a - SupportedGroups
                        ExtensionType::ECPointFormats,          // 0x000b - EC Point Formats (for JA4: 16 extensions)
                        ExtensionType::ALProtocolNegotiation,   // 0x0010 - ALPN
                        ExtensionType::SCT,                     // 0x0012 - SignedCertificateTimestamp
                        ExtensionType::KeyShare,                // 0x0033
                        ExtensionType::PSKKeyExchangeModes,     // 0x002d
                        ExtensionType::SupportedVersions,       // 0x002b
                        ExtensionType::CompressCertificate,     // 0x001b
                        ExtensionType::Unknown(0x44cd),         // 0x44cd - ALPS
                        ExtensionType::Unknown(0xbaba),         // 0xbaba - GREASE
                        ExtensionType::RenegotiationInfo,       // 0xff01 - Renegotiation info
                    ];
                }
            }
