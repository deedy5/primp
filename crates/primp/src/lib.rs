...
//! # primp
//!
//! The `primp` crate provides a convenient, higher-level HTTP
//! [`Client`][client].
//!
...
//! - Cookies
//!
//! The [`primp::Client`][client] is asynchronous (requiring Tokio). For
//! applications wishing  to only make a few HTTP requests, the
//! [`primp::blocking`](blocking) API may be more convenient.
//!
//! Additional learning resources include:
//!
//! - [The Rust Cookbook](https://rust-lang-nursery.github.io/rust-cookbook/web/clients.html)
//! - [primp Repository Examples](https://github.com/seanmonstar/reqwest/tree/master/examples)
//!
//! ## Commercial Support
//!
...
//!
//! ```rust
//! # async fn run() -> Result<(), primp::Error> {
//! let body = primp::get("https://www.rust-lang.org")
//!     .await?
//!     .text()
//!     .await?;
//!
...
//! type, you can use the `primp::Body` constructors.
//!
//! ```rust
//! # use primp::Error;
//! #
...
//! i.e. `let proxy = primp::Proxy::http("https://secure.example")?;`
...
///
/// See also the methods on the [`primp::Response`](./struct.Response.html)
/// type.
///
...
/// ```rust
/// # async fn run() -> Result<(), primp::Error> {
/// let body = primp::get("https://www.rust-lang.org").await?
///     .text().await?;
/// # Ok(())
/// # }
/// ```
...
if_hyper! {
    ...
    pub use self::imp::{Impersonate, ImpersonateOS};
