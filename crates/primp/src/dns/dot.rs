        ...
        self.state
            .get_or_try_init(|| async {
                ...
                let name_servers: Vec<NameServerConfig> = ips
                    .iter()
                    .map(|&ip| {
                        let mut ns = NameServerConfig::tls(ip, self.tls_host.clone().into());
                        // The `tls` constructor hardcodes port 853 in the
                        // connection config; honor the configured custom DoT
                        // port so non-default ports work.
                        for conn in ns.connections.iter_mut() {
                            conn.port = self.tls_port;
                        }
                        ns
                    })
                    .collect();
