...
pub struct DohResolver {
    state: Arc<OnceCell<Arc<TokioResolver>>>,
...
impl DohResolver {
    ...
    pub fn new(url: &str) -> Result<Self, BoxError> {
        ...
        // DoH is HTTPS-only: an `http://` URL would otherwise be silently
        // tunnelled through TLS to port 443; reject at construction.
        if parsed.scheme() != "https" {
            return Err("DoH URL must use the \"https\" scheme".into());
        }
        let host = parsed
            .host_str()
            .ok_or("DoH URL must have a host")?
            .to_string();
            ...
            .get_or_try_init(|| async {
                ...
                let name_servers: Vec<NameServerConfig> = ips
                    ...
                    .map(|&ip| {
                        ...
                        // The `https` constructor hardcodes port 443 in the
                        // connection config; honor the configured custom DoH
                        // port so non-default ports work.
                        for conn in ns.connections.iter_mut() {
                            conn.port = self.doh_port;
                        }
                        ns
                    })
                    .collect();
...
impl Resolve for DohResolver {
    fn resolve(&self, name: Name) -> Resolving {
        ...
        Box::pin(async move {
            ...
            // `opts.timeout` (5s, set in `get_resolver`) bounds the lookup;
            // the cache layer additionally enforces its own
            // `DNS_RESOLUTION_TIMEOUT`. The previously redundant outer
            // `timeout` has been removed to avoid a stacked ~15s worst case.
            let lookup = resolver
...
mod tests {
    ...
    fn new_rejects_non_https_scheme() {
        for url in [
            ...
            "tcp://1.1.1.1/dns-query",
            "ftp://example.com/dns-query",
        ] {
            let err = DohResolver::new(url).unwrap_err();
            assert!(
                err.to_string().contains("https"),
                "expected non-https rejection for {url:?}, got {err}"
            );
        }
        // The canonical form still works and defaults the TLS port.
        let ok = DohResolver::new("https://cloudflare-dns.com/dns-query").unwrap();
        assert_eq!(ok.doh_port, 443);
    }
    ...
    fn builder_creates_with_multi_resolver() {
        let r1: Arc<dyn Resolve> =
            Arc::new(DohResolver::new("https://cloudflare-dns.com/dns-query").unwrap());
        let r2: Arc<dyn Resolve> = Arc::new(crate::dns::gai::GaiResolver::new());
        let client = Client::builder().dns_resolver(vec![r1, r2]).build();
        assert!(client.is_ok());
    }
