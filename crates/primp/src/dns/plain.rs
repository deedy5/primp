...
impl PlainDnsResolver {
    ...
    async fn get_resolver(&self) -> Result<Arc<TokioResolver>, BoxError> {
        // `OnceCell::get_or_try_init` ensures the initializer runs at most once
        // across all concurrent callers; subsequent callers wait for the first
        // call to complete and then reuse the resulting resolver.
        self.state
            .get_or_try_init(|| async {
                ...
                let name_servers: Vec<NameServerConfig> = ips
                    ...
                    .map(|&ip| {
                        ...
                        // The `udp_and_tcp` constructor hardcodes port 53 in the
                        // connection config; honor the configured custom
                        // plain-DNS port so non-default ports work.
                        for conn in ns.connections.iter_mut() {
                            conn.port = self.dns_port;
                        }
                        ns
                    })
                    .collect();
...
impl Resolve for PlainDnsResolver {
    fn resolve(&self, name: Name) -> Resolving {
        ...
        Box::pin(async move {
            ...
            // `DNS_RESOLUTION_TIMEOUT`. A redundant outer `timeout` here
            // previously stacked with those to produce a ~15s worst case,
            // so it has been removed — one timeout budget is sufficient.
            let lookup = resolver
