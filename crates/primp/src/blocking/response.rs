...
impl Response {
    ...
    /// let resp = primp::blocking::get("http://httpbin.org/get")?;
    ...
    /// ```rust
    /// # fn run() -> Result<(), Box<dyn std::error::Error>> {
    /// let resp = primp::blocking::get("http://httpbin.org/redirect/1")?;
    ...
    ///
    /// # fn run() -> Result<(), Error> {
    /// let json: Ip = primp::blocking::get("http://httpbin.org/ip")?.json()?;
    /// # Ok(())
    /// # }
    /// #
    /// # fn main() { }
    ...
    /// ```
    /// # fn run() -> Result<(), Box<dyn std::error::Error>> {
    /// let bytes = primp::blocking::get("http://httpbin.org/ip")?.bytes()?;
    ///
    /// println!("bytes: {bytes:?}");
    /// # Ok(())
    /// # }
    ...
    /// # extern crate primp;
    /// # fn run() -> Result<(), Box<dyn std::error::Error>> {
    /// let content = primp::blocking::get("http://httpbin.org/range/26")?.text()?;
    /// # Ok(())
    /// # }
    /// ```
    ...
    /// # extern crate primp;
    /// # fn run() -> Result<(), Box<dyn std::error::Error>> {
    /// let content = primp::blocking::get("http://httpbin.org/range/26")?
    ...
    /// ```rust
    /// # fn run() -> Result<(), Box<dyn std::error::Error>> {
    /// let mut resp = primp::blocking::get("http://httpbin.org/range/5")?;
    ...
    /// # extern crate primp;
    /// # fn run() -> Result<(), Box<dyn std::error::Error>> {
    /// let res = primp::blocking::get("http://httpbin.org/status/400")?
    ///     .error_for_status();
    /// if let Err(err) = res {
