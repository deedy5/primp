...
async fn test_accept_header_is_not_changed_if_set() {
    ...
    let res = client
        ...
        .header(
            primp::header::ACCEPT,
            primp::header::HeaderValue::from_static("application/json"),
        )
        .send()
        .await
        .unwrap();
...
async fn test_accept_encoding_header_is_not_changed_if_set() {
    ...
    let res = client
        ...
        .header(
            primp::header::ACCEPT_ENCODING,
            primp::header::HeaderValue::from_static("identity"),
        )
        .send()
        .await
        .unwrap();
...
async fn zstd_case(response_size: usize, chunk_size: usize) {
    ...
    let server = server::http(move |req| {
        ...
        async move {
            ...
            let body = primp::Body::wrap_stream(stream.map(Ok::<_, std::convert::Infallible>));
