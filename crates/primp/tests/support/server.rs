...
pub enum Event {
    ConnectionClosed,
    /// A QUIC (HTTP/3) client connection was observed closing from the
    /// server side (emitted when the per-connection accept loop exits).
...
where
    ...
    Fut: Future<Output = http::Response<primp::Body>> + Send + 'static,
{
    let infall = move |req| {
        let fut = func(req);
...
where
    ...
    Fut: Future<Output = Result<http::Response<primp::Body>, E>> + Send + 'static,
    E: Into<Box<dyn std::error::Error + Send + Sync>>,
    F2: FnOnce(&mut Builder) -> Bu + Send + 'static,
{
    http_with_config_and_bind(
        func,
        apply_config,
...
/// Like [`http_with_config`], but binds an explicit address so a test can
/// run servers reachable under distinct hosts (e.g. `127.0.0.1` vs
/// `127.0.0.2`) for host-scoped behavior.
pub fn http_with_config_and_bind<F1, Fut, E, F2, Bu>(
    func: F1,
    apply_config: F2,
...
impl Http3 {
        ...
        Fut: Future<Output = http::Response<primp::Body>> + Send + 'static,
    {
        ...
        let addr = self.addr.unwrap_or_else(|| "127.0.0.1:0".parse().unwrap());
        ...
        thread::spawn(move || {
            ...
            // The QUIC transport uses `quinn`'s bundled `rustls`, which is a
            // separate crate from the patched `primp-rustls` used by the rest
            // of the client. Build the server's TLS config from `quinn`'s rustls
            // types directly so it can be converted into a `QuicServerConfig`.
            let cert_der = quinn::rustls::pki_types::CertificateDer::from(cert);
            let key_der = quinn::rustls::pki_types::PrivateKeyDer::try_from(key)
                            ...
                            tokio::select! {
                                ...
                                Some(accepted) = endpoint.accept() => {
                                    ...
                                    tokio::spawn(async move {
                                        ...
                                        let _ = events_tx.send(Event::QuicConnectionClosed);
                                    });
                                }
                            }
                        }
...
fn install_default_crypto_provider() -> bool {
    // `primp-rustls` is always built with the `aws-lc-rs` crypto provider,
    // so install that for the (plain TLS) HTTP/HTTPS test servers.
    rustls::crypto::aws_lc_rs::default_provider()
        .install_default()
        .expect("failed to install the default aws-lc-rs TLS provider");

    // The QUIC test server is built with `quinn`'s bundled `rustls` (real
    // `rustls` 0.23, using the `aws-lc-rs` provider, as selected by the
    // `rustls-aws-lc-rs` feature in `quinn`'s dependency), which is a
    // separate crate from `primp-rustls` and needs its own crypto provider
    // installed before `quinn::rustls::ServerConfig::builder()` is called.
