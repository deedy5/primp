...
fn test_response_text() {
    ...
    let res = primp::blocking::get(&url).unwrap();
...
fn donot_set_content_length_0_if_have_no_body() {
    ...
    let res = primp::blocking::Client::builder()
        .no_proxy()
        .build()
...
fn test_response_copy_to() {
    ...
    let mut res = primp::blocking::get(&url).unwrap();
    assert_eq!(res.url().as_str(), &url);
    assert_eq!(res.status(), primp::StatusCode::OK);
...
fn test_post() {
    ...
    let res = primp::blocking::Client::new()
        .post(&url)
        .body("Hello")
        .send()
        .unwrap();
...
fn test_error_for_status_4xx() {
    ...
    assert_eq!(err.status(), Some(primp::StatusCode::BAD_REQUEST));
}
...
fn test_error_for_status_5xx() {
    ...
    let err = res.error_for_status().unwrap_err();
    assert!(err.is_status());
    assert_eq!(err.status(), Some(primp::StatusCode::INTERNAL_SERVER_ERROR));
}
...
fn test_appended_headers_not_overwritten() {
    ...
    let client = primp::blocking::Client::new();
...
fn test_blocking_inside_a_runtime() {
    ...
    rt.block_on(async move {
        let _should_panic = primp::blocking::get(&url);
    });
}
...
fn test_allowed_methods_blocking() {
    let resp = primp::blocking::Client::builder()
        .https_only(true)
        .build()
        .expect("client builder")
...
/// Test that a [`primp::blocking::Body`] can be created from [`bytes::Bytes`].
#[test]
...
fn test_response_no_tls_info_for_http() {
    ...
    assert_eq!(res.status(), primp::StatusCode::OK);
    assert_eq!(res.content_length(), Some(5));
    let tls_info = res.extensions().get::<primp::tls::TlsInfo>();
