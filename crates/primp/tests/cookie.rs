...
async fn cookie_store_simple() {
    ...
    let client = primp::Client::builder().cookie_store(true).build().unwrap();
...
#[tokio::test]
async fn one_shot_cookies_without_store_still_sends_cookie() {
    let server = server::http(move |req| async move {
        assert_eq!(req.headers()["cookie"], "oneshot=1");
        http::Response::builder()
            .header("content-type", "text/plain")
            .body("handler-ran".into())
            .unwrap()
    });

    // No cookie store configured: the one-shot cookies must still reach the
    // wire — they are an explicit per-request header, not a jar operation.
    let client = primp::Client::new();

    let url = format!("http://{}/", server.addr());
    let res = client
        .get(&url)
        .one_shot_cookies(http::HeaderValue::from_static("oneshot=1"))
        .send()
        .await
        .unwrap();
    assert_eq!(res.text().await.unwrap(), "handler-ran");
}
