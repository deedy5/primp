...
mod tests {
    ...
    async fn sender_checks_for_want_on_send() {
        ...
        drop(tx.try_send(Custom(1)).expect("1 buffered"));
        ...
        drop(tx.try_send(Custom(2)).expect("2 ready"));
    }
    ...
    fn unbounded_sender_doesnt_bound_on_want() {
        ...
        drop(tx.try_send(Custom(1)).unwrap());
        drop(tx.try_send(Custom(2)).unwrap());
        drop(tx.try_send(Custom(3)).unwrap());
