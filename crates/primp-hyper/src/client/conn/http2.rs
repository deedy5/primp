    ...
    Ex: Clone,
{
    ...
    ///
    /// Default is to not send the setting, allowing the peer's default.
    ...
    /// Sets whether to enable server push.
    ///
    /// This setting can be used to disable server push. Clients SHOULD
    /// set this to false, as server push is rarely used and can
    /// be a security risk.
    ///
    ...
    /// or testing purposes.
    pub fn headers_pseudo_order(&mut self, order: h2::frame::PseudoOrder) -> &mut Self {
        self.h2_builder.headers_pseudo_order = Some(order);
        self
    }
    ...
    pub fn headers_order(&mut self, order: Vec<http::HeaderName>) -> &mut Self {
        self.h2_builder.headers_order = Some(order);
        self
    }
    ...
    pub fn settings_order(&mut self, order: h2::frame::SettingsOrder) -> &mut Self {
        self.h2_builder.settings_order = Some(order);
        self
    }

    /// Sets whether to disable RFC 7540 priorities.
    ///
    ...
    pub fn no_rfc7540_priorities(&mut self, enabled: bool) -> &mut Self {
        self.h2_builder.no_rfc7540_priorities = Some(enabled);
        self
    }

    /// Sets the `SETTINGS_ENABLE_CONNECT_PROTOCOL` value.
    ///
    /// This corresponds to the `SETTINGS_ENABLE_CONNECT_PROTOCOL` (0x8) setting.
    /// When enabled, the client advertises support for Extended CONNECT (RFC 8441).
    ///
    /// Default is `None` (not set).
    pub fn enable_connect_protocol(&mut self, val: bool) -> &mut Self {
        self.h2_builder.enable_connect_protocol = Some(val);
        self
    }
