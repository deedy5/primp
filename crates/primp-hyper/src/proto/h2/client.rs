...
const _DEFAULT_MAX_HEADER_LIST_SIZE: u32 = 1024 * 16; // 16kb  !!! Not used for now
...
fn new_builder(config: &Config) -> Builder {
    ...
    if let Some(enable) = config.no_rfc7540_priorities {
        builder.no_rfc7540_priorities(enable as u32);
    }
    if let Some(enable) = config.enable_connect_protocol {
        builder.enable_connect_protocol(enable as u32);
    }
    ...
    if let Some(ref order) = config.headers_order {
        builder.headers_order(order.clone());
    }
    if let Some(data) = config.headers_priority {
        builder.headers_priority(Some(data));
    }
