...
pub struct Builder {
    ...
    headers_pseudo_order: Option<crate::frame::PseudoOrder>,

    /// Whether to include PRIORITY flag in HEADERS frames and its parameters.
    /// None = no PRIORITY, Some((weight, dep, exclusive)) = PRIORITY with those values.
...
impl Builder {
    ...
    pub fn settings_order(&mut self, order: crate::frame::SettingsOrder) -> &mut Self {
        self.settings.set_settings_order(order);
        self
    }

    /// Sets the NO_RFC7540_PRIORITIES setting.
    ///
    /// This setting indicates that the sender does not support RFC 7540 priorities.
    /// When set to 1, the sender is indicating it does not use the priority
    /// mechanism described in RFC 7540.
    pub fn no_rfc7540_priorities(&mut self, enabled: u32) -> &mut Self {
        self.settings.set_no_rfc7540_priorities(Some(enabled));
        self
    }

    /// Sets the ENABLE_CONNECT_PROTOCOL setting.
    ///
    /// This setting enables the use of the extended CONNECT method
    /// (RFC 8441). When set to 1, the sender indicates support for
    /// extended CONNECT.
    pub fn enable_connect_protocol(&mut self, val: u32) -> &mut Self {
        self.settings.set_enable_connect_protocol(Some(val));
        self
    }
    ...
    /// or testing purposes.
    pub fn headers_pseudo_order(&mut self, order: crate::frame::PseudoOrder) -> &mut Self {
        self.headers_pseudo_order = Some(order);
        self
    }

    /// Sets PRIORITY flag parameters for HEADERS frames.
    ///
    /// When set, HEADERS frames will include priority data.
    /// None = no PRIORITY, Some((weight, dep, exclusive)) = PRIORITY with those values.
    pub fn headers_priority(&mut self, data: Option<(u8, u32, bool)>) -> &mut Self {
        self.headers_priority = data;
        self
    }
    ...
    pub fn headers_order(&mut self, order: Vec<http::HeaderName>) -> &mut Self {
        self.headers_order = Some(order);
        self
    }
    ...
    B: Buf,
{
    async fn handshake2(
        ...
        tracing::debug!(
            "handshake2: settings header_table_size={:?}, enable_push={:?}, initial_window_size={:?}, max_frame_size={:?}, max_header_list_size={:?}",
            builder.settings.header_table_size(),
            builder.settings.is_push_enabled(),
            builder.settings.initial_window_size(),
            builder.settings.max_frame_size(),
            builder.settings.max_header_list_size(),
        );
        ...
        tracing::debug!("handshake2: sending initial settings frame");
        codec
...
impl Peer {
    pub fn convert_send_message(
        ...
        end_of_stream: bool,
        pseudo_order: Option<crate::frame::PseudoOrder>,
