...
pub struct Iter {
    ...
    /// Header fields (sorted by header_order if set)
    fields: std::vec::IntoIter<(Option<HeaderName>, HeaderValue)>,
...
struct HeaderBlock {
    ...
    /// Optional ordering for regular headers (for browser fingerprinting).
...
impl Iterator for Iter {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if let Some(ref mut pseudo) = self.pseudo {
            // Iterate through the configured order
            for id in &self.pseudo_order.ids {
                match id {
                    PseudoId::Method => {
...
impl HeaderBlock {
    ...
    fn into_encoding(self, encoder: &mut hpack::Encoder) -> EncodingHeaderBlock {
        ...
        let mut fields_vec: Vec<(Option<HeaderName>, HeaderValue)> =
            self.fields.into_iter().collect();

        // Sort by header_order if specified (for browser fingerprinting)
        if let Some(ref order) = self.header_order {
            let order_map: std::collections::HashMap<&HeaderName, usize> = order
                .iter()
                .enumerate()
                .map(|(i, name)| (name, i))
                .collect();
