...
impl Builder {
    ...
    /// Sets the maximum size of the local header compression table used by
    /// HPACK to compress header fields.
    ///
    /// This setting informs the peer of the maximum size of the header compression
    /// table used to decode header blocks.
    ///
    /// Default is currently 4,096 bytes, but can change.
    #[cfg(feature = "http2")]
    #[cfg_attr(docsrs, doc(cfg(feature = "http2")))]
    ...
    /// Sets the maximum number of concurrent streams that the client will allow
    /// the server to initiate.
    ///
    /// This setting is typically used by servers to limit the number of streams
    /// a client can create. For clients, this limits the number of streams the
    /// server can push (if push is enabled).
    ///
    /// Default is currently unset (no limit), but can change.
    #[cfg(feature = "http2")]
    #[cfg_attr(docsrs, doc(cfg(feature = "http2")))]
    ...
    /// Sets whether to enable HTTP/2 server push.
    ///
    /// This setting can be used to disable server push. Clients SHOULD
    /// set this to false, as server push is rarely used and can be
    /// a security risk.
    ///
    ...
    #[cfg(feature = "http2")]
    #[cfg_attr(docsrs, doc(cfg(feature = "http2")))]
    pub fn http2_headers_pseudo_order(&mut self, order: hyper::PseudoOrder) -> &mut Self {
        self.h2_builder.headers_pseudo_order(order);
        self
    }

    /// Sets the HTTP/2 SETTINGS frame order for the connection preface.
    ///
    /// This determines the order in which settings are sent in the initial SETTINGS frame.
    /// Customizing the order may be useful for testing or protocol compliance.
    #[cfg(feature = "http2")]
    #[cfg_attr(docsrs, doc(cfg(feature = "http2")))]
    pub fn http2_settings_order(&mut self, order: hyper::SettingsOrder) -> &mut Self {
        self.h2_builder.settings_order(order);
        self
    }

    /// Sets whether to disable RFC 7540 priorities for HTTP/2.
    ///
    /// This corresponds to the `SETTINGS_NO_RFC7540_PRIORITIES` (0x9) setting.
    /// When enabled, the client will not send priority frames and will ignore
    /// priority frames received from the server.
    ///
    /// Default is `false`.
    #[cfg(feature = "http2")]
    #[cfg_attr(docsrs, doc(cfg(feature = "http2")))]
    pub fn http2_no_rfc7540_priorities(&mut self, enabled: bool) -> &mut Self {
        self.h2_builder.no_rfc7540_priorities(enabled);
        self
    }

    /// Sets whether to include PRIORITY flag in HTTP/2 HEADERS frames.
    ///
    /// When enabled, HEADERS frames will include priority data (weight=0,
    /// stream_dependency=0, exclusive=1) matching Chrome's HTTP/2 behavior.
    ///
    /// Default is `false`.
    #[cfg(feature = "http2")]
    #[cfg_attr(docsrs, doc(cfg(feature = "http2")))]
    pub fn http2_headers_priority(&mut self, data: Option<(u8, u32, bool)>) -> &mut Self {
        self.h2_builder.headers_priority(data);
        self
    }
    ...
    #[cfg(feature = "http2")]
    #[cfg_attr(docsrs, doc(cfg(feature = "http2")))]
    pub fn http2_headers_order(&mut self, order: Vec<http::HeaderName>) -> &mut Self {
        self.h2_builder.headers_order(order);
        self
    }

    /// Sets the HTTP/2 SETTINGS_ENABLE_CONNECT_PROTOCOL value.
    ///
    /// When enabled, the client advertises support for Extended CONNECT
    /// protocol (RFC 8441).
    ///
    /// Default is `None` (not sent).
    #[cfg(feature = "http2")]
    #[cfg_attr(docsrs, doc(cfg(feature = "http2")))]
    pub fn http2_enable_connect_protocol(&mut self, val: u32) -> &mut Self {
        self.h2_builder.enable_connect_protocol(val != 0);
        self
    }

    /// Sets the initial HTTP/2 stream ID for the connection.
    ///
    /// Firefox starts at stream ID 3 (skipping stream 1), Chrome uses the default of 1.
    ///
    /// Default is 1 (standard HTTP/2).
    #[cfg(feature = "http2")]
    #[cfg_attr(docsrs, doc(cfg(feature = "http2")))]
