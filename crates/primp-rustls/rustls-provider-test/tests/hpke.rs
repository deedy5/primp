    ...
    for (idx, vec) in test_vectors().into_iter().enumerate() {
        ...
        for enc in vec.encryptions {
            ...
            for (sealer, opener) in &hpke_pairs {
                let (enc, ciphertext) = sealer.seal(&info, &aad, &pt, &pk_r).unwrap();

                let plaintext = opener.open(&enc, &info, &aad, &ciphertext, &sk_r).unwrap();
...
impl TestVector {
    ...
    fn applicable(&self) -> Option<Vec<(&'static dyn Hpke, &'static dyn Hpke)>> {
        ...
        match Self::lookup_suite(self.suite(), aws_lc_rs::hpke::ALL_SUPPORTED_SUITES) {
