...
/// in hybrid with X25519 when the prefer-post-quantum feature is enabled.
...
mod tests {
    ...
    fn default_suites_are_fips() {
        assert!(super::DEFAULT_CIPHER_SUITES.iter().all(|scs| scs.fips()));
    }
    ...
    fn certificate_sig_algs() {
        ...
        assert_eq!(
            super::SUPPORTED_SIG_ALGS
                ...
                .map(|alg| {
                    (
                        alg.public_key_alg_id().as_ref().to_vec(),
