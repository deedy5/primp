...
#[cfg(feature = "zlib")]
mod feat_zlib_rs {
    use zlib_rs::{compress_bound, compress_slice, decompress_slice, InflateConfig, ReturnCode};
    ...
    impl CertCompressor for ZlibRsCompressor {
        fn compress(
            ...
            let config = match level {
                CompressionLevel::Interactive => zlib_rs::DeflateConfig::default(),
                CompressionLevel::Amortized => zlib_rs::DeflateConfig::new(Z_BEST_COMPRESSION),
            };
...
mod feat_zstd {
    ...
    /// A certificate decompressor for the zstd algorithm.
    ...
    impl CertDecompressor for ZstdDecompressor {
        fn decompress(&self, input: &[u8], output: &mut [u8]) -> Result<(), DecompressionFailed> {
            let mut decoder = zstd::stream::Decoder::new(input).map_err(|_| DecompressionFailed)?;
            match std::io::Read::read_exact(&mut decoder, output) {
                Ok(()) => Ok(()),
                Err(_) => Err(DecompressionFailed),
            }
        }
    ...
    /// A certificate compressor for the zstd algorithm.
    ...
    impl CertCompressor for ZstdCompressor {
        fn compress(
            ...
            let level = match level {
                ...
                CompressionLevel::Amortized => 3,
            };
            zstd::encode_all(input.as_slice(), level).map_err(|_| CompressionFailed)
        }
