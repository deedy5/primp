...
extension_struct! {
    ...
    pub(crate) struct ClientExtensions<'a> {
        ...
        /// Maximum fragment length (RFC6066)
        ExtensionType::MaxFragmentLength =>
            pub(crate) max_fragment_length: Option<u8>,
        ...
        /// Record size limit extension (RFC8449) - Chrome 144 fingerprinting
        ...
        /// Application Settings (ALPS) extension for Chrome fingerprinting
        ...
        /// Unknown extensions with their payloads for encoding.
        /// Used for GREASE and other non-standard extensions.
        pub(crate) unknown_extensions: Vec<(ExtensionType, Payload<'a>)>,

        /// When true, ECH extension is inserted before trailing GREASE extensions
        /// in contiguous_extensions (for Chrome fingerprinting compatibility).
        pub(crate) ech_before_trailing_grease: bool,
    }
}
...
impl<'a> Codec<'a> for ClientExtensions<'a> {
    fn encode(&self, bytes: &mut Vec<u8>) {
        ...
        if order.is_empty() && self.unknown_extensions.is_empty() {
            return;
        }

        let body = LengthPrefixedBuffer::new(ListLength::U16, bytes);
        for item in order {
            // First, try to encode as a known extension
            self.encode_one(item, body.buf);

            // If it's an unknown extension in contiguous_extensions, encode it here
            // This ensures unknown extensions are in their proper position
            if self.contiguous_extensions.contains(&item) {
                for (ext_type, payload) in &self.unknown_extensions {
                    if *ext_type == item {
                        ext_type.encode(body.buf);
                        payload.encode(LengthPrefixedBuffer::new(ListLength::U16, body.buf).buf);
                    }
                }
            }
        }
