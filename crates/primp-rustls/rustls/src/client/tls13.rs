...
pub(super) fn initial_key_share(
    ...
    let group = config
        ...
        .unwrap_or_else(|| {
            // When browser emulation is enabled, use the first named_group that has
            // a matching kx_group in the provider. This ensures the key_share matches
            // the named_groups advertised in the ClientHello.
            #[cfg(feature = "impersonate")]
            if let Some(be) = &config.browser_emulation {
                if let Some(named_groups) = &be.named_groups {
                    for named_group in named_groups {
                        // Skip GREASE - it's not a real key exchange group
                        // Check both NamedGroup::GREASE and Unknown GREASE values (0x?A?A pattern)
                        let is_grease = match *named_group {
                            crate::msgs::enums::NamedGroup::GREASE => true,
                            crate::msgs::enums::NamedGroup::Unknown(v) => {
                        ...
                        if let Some(skxg) =
                            config.find_kx_group(*named_group, ProtocolVersion::TLSv1_3)
                        {
                            return skxg;
                        }
                    }
            ...
            // Fall back to the first kx_group from the provider
            config
                .provider
                .kx_groups
                .iter()
